...
@section('admin')
@php  $locale = Session::get('locale', config('app.locale')); @endphp
    <section class="content-header">
       <div class="container-fluid d-flex justify-content-between align-items-center">
            <div>
                <h1 class="m-0">Editing gallery for campaign: {{ $campaign->title }}</h1>
                <small class="text-muted">
                    Drag and drop the images to re-order the elements.
                </small>
                <ol class="m-0 mt-2 breadcrumb">
                    <li class="breadcrumb-item"><a href="{{ url('/admin') }}">Dashboard</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('campaign.gallery.index') }}">Gallery Editor</a></li>
                ...
                <div class="card-body">
                    ...
                    <div class="gallery-grid" id="galleryGrid">
                        ...
                        @foreach ($campaign->galleryImages as $img)
                               ...
                               {{-- 🔝 Top Action Bar --}}
                                <div style="position:absolute; top:8px; left:8px; right:8px; display:flex; justify-content:space-between; align-items:center; pointer-events:auto;">
                                    ...
                                    <button type="button" class="btn btn-primary btn-sm"
                                        data-toggle="modal"
                                        data-target="#modal-default-{{ $img->id }}">
                                        <i class="fas fa-pen"></i>
                                    </button>

                                    {{-- 👁️ Preview (Middle) --}}
                                    <a href="{{ route('gallery.image.details', ['slug' => $img->slug ?? $img->id]) }}" target="_blank"
                            ...
                            @include('admin.campaign_gallery.edit_modal')
                        @endforeach
    ...
    <div class="modal fade" id="uploadModal" tabindex="-1" role="dialog" aria-hidden="true">
        <div class="modal-dialog modal-md" role="document">
            <form class="modal-content" method="POST" action="{{ route('campaign.gallery.store', $campaign) }}"
                ...
                <div class="modal-body">
                    ...
                    @foreach (App\Models\Language::where('active', 1)->get() as $key => $language)
                    @if ($language->language_code === $locale)
                        <div class="form-group">
                            <label>Title ({{ $language->title }})</label>
                            <input type="text" name="title[{{ $language->language_code }}]" class="form-control"
                                required placeholder="Enter title {{ $language->language_code }}">
                        </div>

                          <div class="form-group"> 
                                <label>Alt Text ({{ $language->title }})</label>
                                <input type="text" name="alt_text[{{ $language->language_code }}]"
                                    class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>Captions ({{ $language->title }})</label>
                                <input type="text" name="caption[{{ $language->language_code }}]"
                                    class="form-control" required>
                            </div>
                            <div class="form-group">
                                <label>Tags ({{ $language->title }})</label>
                                <input type="text" name="tags[{{ $language->language_code }}]"
                                    class="form-control" required>
                            </div>

                        <div class="form-group">
                            <label>Description ({{ $language->title }})</label>
                            <textarea name="description[{{ $language->language_code }}]" id="description" class="form-control" required
                                placeholder="Enter title {{ $language->language_code }}"></textarea>
                        </div> 
                    @endif
                    @endforeach
    ...
    {{-- Hidden delete forms --}}
    @foreach ($campaign->galleryImages as $img)
        <form id="deleteForm{{ $img->id }}" method="POST"
            action="{{ route('campaign.gallery.destroy', [$campaign, $img]) }}" style="display:none;">
            @csrf
            @method('DELETE')
        </form>
    @endforeach
...
@section('js')
    ...
    <script>
        ...
        document.getElementById('btnSave').addEventListener('click', async function() {
            ...
            try {
                const res = await fetch("{{ route('campaign.gallery.reorder', $campaign) }}", {
                    method: 'POST',
                    headers: {
