...
@section('meta')
  ...
  {{-- Open Graph --}}
  <meta property="og:title" content="{{ translate('Contact Us page title') }}">
  <meta property="og:description" content="{{ translate('Contact Us page description') }}">
  ...
  <meta name="twitter:title" content="{{ translate('Contact Us page title') }}">
  <meta name="twitter:description" content="{{ translate('Contact Us page description') }}"> 
  ...
  <div class="max-w-6xl md:px-4 md:mx-auto lg:px-10 md:pt-48">
    ...
    <div class="grid overflow-hidden bg-white shadow-sm lg:grid-cols-2 md:rounded-xl">
      ...
      <div class="relative min-h-[320px] lg:min-h-[420px]">
        <img src="{{ asset('storage/contact_us_cover_image/'.$setting->contact_us_cover_image) }}"
             class="absolute inset-0 object-cover w-full h-full" alt="{{ translate('contact us cover image alt') }}" title="{{ translate('Annual report cover image title') }}" /> 
        ...
        <div class="absolute inset-0 flex flex-col justify-between p-10">
          <div>
            <h2 class="text-2xl sm:text-4xl font-semibold leading-tight text-[#E9D48A] whitespace-pre-line">
              @if(!empty($setting->contact_us_title))
          ...
          <p class="max-w-sm text-sm leading-relaxed text-white/85 whitespace-pre-line">
            @if(!empty($setting->contact_us_content))
              {!! $setting->contact_us_content !!}
            @endif
          </p>
        </div>
      </div>
      ...
      <div class="bg-[#D94A4A] p-8 sm:p-10 lg:p-12">
        @if(session('success'))
            <p class="mb-2 text-white">{{ session('success') }}</p>
        @endif
 ...
 <div class="max-w-6xl mx-5 mx-auto px-16 py-5 w-full md:mx-auto">
    ...
    <hr>
      ...
      <div class="grid grid-cols-1 gap-10 text-sm sm:grid-cols-2 md:grid-cols-3 md:w-3/4">
        ...
        <!-- Email Section -->
        <div class="space-y-1">
          <h3 class="text-base font-bold text-red-600"> {{ translate('Email Address') }}</h3>
          <p class="text-gray-700">{{ $setting->telephone }}</p>
          <p class="text-gray-700">{{ $setting->email }}</p>
        </div>

        <!-- Address Section -->
        <div class="space-y-1">
          <h3 class="text-base font-bold text-red-600"> {{ translate('Address') }}</h3>
  ...
  <div class="max-w-6xl px-4 py-5 mx-auto">
    <div class="grid gap-6 md:grid-cols-3">
      <div class="bg-[#fbe8e8] p-6 rounded-md">
        <h3 class="text-lg font-bold"> {{ translate('START VOLUNTEERING') }}</h3>
      ...
      <div class="bg-[#fbe8e8] p-6 rounded-md">
        <h3 class="text-lg font-bold "> {{ translate('DOWNLOAD ANNUAL REPORT') }}</h3>
        <p class="mt-3 text-sm whitespace-pre-line">
  ...
  <div class="max-w-6xl px-4 py-10 mx-auto space-y-10">
    <div class="grid items-center gap-8 md:grid-cols-2">
      ...
      <div class="space-y-8 text-sm">
        <div>
          <h3 class="font-bold text-red-600"> {{ translate('Our offices in the USA:') }}</h3>
          <div  class="whitespace-pre-line">
          @if(!empty($setting->office_usa))
              {!! $setting->office_usa !!}
          @endif
          </div>
        </div>
        ...
        <div>
          <h3 class="font-bold text-red-600"> {{ translate('Our Offices in Venezuela:') }}</h3>
          <div  class="whitespace-pre-line">
          @if(!empty($setting->office_venezuela))
              {!! $setting->office_venezuela !!}
          @endif
          </div>
        </div>
      </div>
