...
<head>
  ...
  @if(!empty($setting?->gsc_verification))
    {!! $setting->gsc_verification !!}
  @endif
...
<!-- Google Tag Manager (noscript) -->
  @if(!empty($setting?->google_tag_manager_body))
  ...
  {{-- <div class="w-full md:container md:mx-auto px-4-"> --}}
...
<audio id="chatPing" preload="auto">
  <source src="{{ asset('sounds/notify.wav') }}" type="audio/mpeg">
</audio>
...
(function () {
  ...
  function fetchNew(){
    ...
    $.get("{{ route('guest.chat.fetch') }}", { after_id: lastId })
      .done(res => {
  ...
  function send(){
    ...
    $.post("{{ route('guest.chat.send') }}", { message: text })
      .done(res => {
