...
@section('title', $campaign->seo_title)
@section('meta_description', $campaign->seo_description)
@section('meta_keyword',  $campaign->meta_keyword ?? translate('Home page meta keyword'))
...
@include('frontend.seo.campaign_details')
...
<section class="pb-5- -mt-48 relative z-10">
    ...
    <div class="max-w-7xl md:mx-auto md:px-4- grid lg:grid-cols-[3fr,1fr] gap-6 my-3">
        <div class="">
            ...
            <span class="shrink-0 px-6 py-1 font-semibold rounded-tr-lg rounded-bl-lg mr-3"
                    style="background-color: {{ $item->slogan_bg_color ?? 'black' }}; color: {{ $item->slogan_text_color ?? 'white' }};">
                        {{ $item->slogan_title }}
            </span>
            @endforeach
        </div>
    ...
    <div class="max-w-7xl md:mx-auto md:px-4- grid lg:grid-cols-[3fr,1fr] gap-6">
           ...
           <!-- ✅ RESPONSIVE CARD (desktop + mobile like screenshot) -->
            <div class="bg-white shadow-md border border-gray-200 overflow-hidden
                ...
                <div class="relative bg-[#D94647] text-white px-5 md:px-8 py-3 overflow-hidden">
                    ...
                    {{ \Illuminate\Support\Str::limit($campaign->title ?? '', 100) }}
                    </h1>
                   ...
                   <a href="javascript:void(0)"
                    ...
                    <div class="flex flex-wrap items-center gap-4 md:gap-6 mb-4 text-[12px] text-gray-700 justify-center md:justify-start">
                        ...
                        onmouseenter="openCertModal(this,
                            ...
                            @js(asset('storage/company_logo/'.$icon->company_logo))  {{-- ✅ badge icon --}}
                        )"
                    ...
                    >
                        <span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-white border border-gray-200 overflow-hidden">
                            <img
                                src="{{ asset('storage/campaign_icon/'.$icon->campaign_icon) }}"
            ...
            <div class="bg-white shadow-md border border-gray-200 md:mx-5 md:mx-0 lg:mx-0">
                ...
                <div class="tab-panel px-8 py-8 text-gray-700 space-y-6 bg-[#fafafa]"
                    ...
                    <section class="grid md:grid-cols-[2.2fr,1.1fr] gap-3 items-stretch mt-4">
                        <div class="relative bg-gray-200 h-56 md:h-64 overflow-hidden rounded-sm">
                            @foreach($campaign->galleryImages->take(1) as $galItem)
                                @php
                                ...
                                'ext' => $ext,
                            ]) }}" class="w-full h-full object-cover"  alt="{{ $galItem->alt_text ?? $galItem->title ?? 'Gallery Image' }}" title="{{ $galItem->title ?? $galItem->alt_text ?? 'Gallery Image' }}">
                            @endforeach
                        ...
                        <div class="grid grid-rows-3 gap-2 h-56 md:h-64">
                            ...
                            @foreach($campaign->galleryImages->skip(1)->take(3) as $galItem)
                            @php
                                // title JSON হলে locale অনুযায়ী text বের করো
                                $title = $galItem->title;
                                if (is_string($title)) {
                            ...
                            <img src="{{ route('gallery.image', [
                                ...
                                'imageSlug' => $galItem->slug,
                                'ext' => $ext,
                            ]) }}" class="w-full h-full object-cover rounded-sm" alt="{{ $galItem->alt_text ?? $galItem->title ?? 'Gallery Image' }}" title="{{ $galItem->title ?? $galItem->alt_text ?? 'Gallery Image' }}">
                            @endforeach
                    ...
                    @if(!empty($campaign->standard_webpage_content))
                        <section class="space-y-2 prose max-w-none">
                            {!! $campaign->standard_webpage_content !!}
                        </section>
                    @endif

                    @if(!empty($campaign->problem))
                        <section class="space-y-2 prose max-w-none">
                            {!! $campaign->problem !!}
                        </section>
                    @endif

                    @if(!empty($campaign->solution))
                        <section class="space-y-2 prose max-w-none">
                            {!! $campaign->solution !!}
                        </section>
                    @endif

                    @if(!empty($campaign->impact))
                        <section class="space-y-2 prose max-w-none">
                            {!! $campaign->impact !!}
                        </section>
                    @endif
                ...
                {{-- GALLERY CONTENT --}}
                {{-- =========================
                GALLERY TAB + LIGHTBOX (BIG IMAGE + CAPTION + NEXT/PREV UNDER DESC + THUMBS UNDER THAT)
                ========================= --}}

                {{-- ===================== GALLERY TAB + IMAGE-WISE LIGHTBOX (FULL) ===================== --}}
                <div class="tab-panel px-8 py-8 bg-[#fafafa] hidden" data-tab-panel="gallery">
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                        @foreach ($campaign->galleryImages as $i => $galItem)
                            @php
                                // title JSON হলে locale অনুযায়ী text বের করো
                                $title = $galItem->title;
                                if (is_string($title)) {
                            ...
                            <img src="{{ route('gallery.image', [
                                ...
                                class="gallery-thumb" alt="{{ $galItem->alt_text ?? $galItem->title ?? 'Gallery Image' }}" title="{{ $galItem->title ?? $galItem->alt_text ?? 'Gallery Image' }}">
                        @endforeach
            ...
            @endphp
                ...
                @include('frontend.includes.gallery_light_box')
                            ...
                            <article class="bg-white shadow border border-gray-200 flex flex-col">
                                ...
                                <div class="p-4 flex-1 flex flex-col">
                                    <h4 class="font-semibold text-sm mb-2 text-gray-800">
                                        {{ \Illuminate\Support\Str::limit($story->title ?? '', 40) }}
                                    </h4>
                        ...
                        <article
                            ...
                            <div class="w-full md:w-1/2 bg-[#f04848] text-white px-8 py-8 flex flex-col justify-center">
                                <p class="text-xs font-semibold uppercase tracking-[0.03em] mb-4">
                                     {{ $campaign->footer_title }} 
                                </p> 

                                <h3 class="text-xl md:text-2xl font-semibold leading-snug tracking-[0.03em] uppercase">
                                    {!! $campaign->footer_subtitle !!} 
                                </h3>

                                <a  href="{{ $campaign->footer_button_link }}" class="mt-6 inline-flex items-center justify-center bg-white text-[#f04848]
        ...
        {{-- ====== SIDEBAR DONATION WIZARD (DESIGN SAME + NEXT/PREV + CUSTOM AMOUNT INPUT) ====== --}}
        @include('frontend.includes.donate_box')
...
{{-- ===================== WIZARD SCRIPT (Next/Prev + Custom Amount + Payment Tabs) ===================== --}}
<script>
...
{{-- =================== SCRIPT (Company field toggle only) =================== --}}
<script>
  document.addEventListener('DOMContentLoaded', () => {
...
{{-- ===================== SCRIPT ===================== --}}
<script>
    window.__GALLERY_ITEMS__ = @json($galleryItems);
